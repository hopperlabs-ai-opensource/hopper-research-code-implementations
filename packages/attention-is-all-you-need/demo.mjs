...
function showStep() {
	const steps = [
		[
			...
			mode === "uniform"
				? "Equal attention replaces the query with zeros. Every dot product is zero, so every record receives the same weight."
				: mode === "untrained"
					? "The chosen person selects a random query vector. Keys are also random: no training updates have been applied."
...
function drawHeatmap() {
	...
	el("heatmap-body").replaceChildren(
		...records.map((row) => {
			const tr = document.createElement("tr");
			tr.dataset.selected = String(row.person === el("person").value);
			const th = document.createElement("th");
			th.scope = "row";
			th.textContent = row.person;
			tr.append(th);
			...
			for (const weight of result.weights) {
				const td = document.createElement("td");
				td.dataset.level = Math.min(5, Math.floor(weight * 6));
...
function labInspect() {
	return {
		...
		output: structuredClone(computed),
	};
}
function labRun(input) {
	lookupDelivery(
		input.mode === "untrained" ? untrainedModel : trainedModel,
		input.records,
		input.person,
	...
	el("person").value = input.person;
	drawRecords();
	render();
	return labInspect();
}
function labReset() {
	return labRun({ records: deliverySample, person: "Mira", mode: "trained" });
}
